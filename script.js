// Lista de símbolos (clases CSS, no imágenes sueltas)
const symbolList = [
...
// Crear carretes con símbolos desde spritesheet
function createReel(reel) {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        symbolEl.className = `symbol ${randomSymbol}`;
...
function spinReel(reel, duration, delay) {
    return new Promise(resolve => {
        ...
        function animate() {
            ...
            if (pos >= reel.scrollHeight / 2) pos = 0; // loop
...
spinBtn.addEventListener("click", async () => {
    ...
    // Spin reels con delay
    for (let i = 0; i < reels.length; i++) {
        await spinReel(reels[i], 1500 + i * 500, i * 200);
    }

    // Verificar premio (compara clases)
    let result = reels.map(r => r.children[2].classList[1]);
